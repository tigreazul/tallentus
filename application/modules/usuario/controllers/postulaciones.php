...
class Postulaciones extends MX_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('postulaciones_model','postulaciones');
        ...
        if($this->id_usuario == ''){
            ...
            if(count($arrRes) != 0){
                switch ($arrRes['per_ver']) {
                    ...
                    case '1':
                        #######################################
                        $empresa = $this->postulaciones->lst_postulaciones();

                        $data = array(
                            'empresa'    => $empresa,
                            'page_title' => 'Postualciones',
                            'module'     => 'usuario',
                            'view_file'  => 'postulaciones/index_view'
                            );
        ...
        if($this->id_usuario == ''){
            ...
            if(count($arrRes) != 0){
                switch ($arrRes['per_delete']) {
                    ...
                    case '1':
                        #######################################
                        $usuario = $this->postulaciones->_obtener_datos_postulaciones((int)$id);
                        if(count($usuario) != 0){
                            $arr_usuario = array(
                                'postu_estado' => 0
                                );
                            $this->postulaciones->_update_postulaciones($arr_usuario,(int)$id);
                        }
                        redirect('usuario/postulaciones','refresh');
                        ################
                    break;
                }
            }else{
